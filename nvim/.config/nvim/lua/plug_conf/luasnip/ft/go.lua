...
local shared = require "plug_conf.t_luasnip_dir"
...
local transform = function(text, info)
  ...
  elseif text == "error" then
    if info then
      ...
      return c(
        info.index,
        { t(string.format('errors.Wrap(%s, "%s")', info.err_name, info.func_name)), t(info.err_name) }
      )
    else
      return t "err"
    end
...
local go_ret_vals = function(args)
  return snippet_from_nodes(nil, go_result_type { index = 0, err_name = args[1][1], func_name = args[2][1] })
end
...
ls.add_snippets("go", { s("f", fmt("func {}({}) {} {{\n\t{}\n}}", { i(1, "name"), i(2), i(3), i(0) })) })
